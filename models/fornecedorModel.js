...
const Fornecedor = {
    create: (fornecedor, callback) => {
        const query = 'INSERT INTO fornecedores (nome) VALUES (?)';
        db.query(query, [fornecedor.nome], (err, results) => {
            if (err) {
                return callback(err);
            }
    ...
    findByFornecedorname: (nome, callback) => {
    ...
    update: (id, fornecedor, callback) => {
        const query = 'UPDATE fornecedores SET nome = ? WHERE id = ?';
        db.query(query, [fornecedor.nome,id], (err, results) => {
            if (err) {
                return callback(err);
            }
